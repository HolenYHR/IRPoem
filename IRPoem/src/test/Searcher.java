...
public class Searcher {
	...
	public void search(String key) throws ParseException, IOException, InvalidTokenOffsetsException {
		System.out.println("Searcher:" + key);
		...
		for (ScoreDoc scoreDoc : hits.scoreDocs) {
			...
			if (id.trim().equals(temp.trim()))
				continue;
			temp = id;

			// Printer.print(doc.get("id")+"\t");
			// Printer.print(doc.get("name")+"\t"+doc.get("chaodai")+"\t"+doc.get("zuozhe")+"\t");
			...
			Poem p = new Poem(Integer.parseInt(doc.get("id")), doc.get("name"), doc.get("chaodai"), doc.get("zuozhe"),
					doc.get("content"));
			System.out.println(p.getName());
			Printer.add(p);
		}
		reader.close();
		System.out.println(Printer.getPoems().size());
	}
